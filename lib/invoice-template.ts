...
export function getMonthlyInvoiceBreakdown(invoice: Invoice): MonthlyInvoiceBreakdown {
  ...
  for (const line of getInvoiceLines(invoice)) {
    ...
    if (line.contract_id || /^mantenimiento (piscina|mensual)/i.test(line.description)) {
...
function replaceInvoiceBranding(document: string) {
  return document
    .replaceAll('Concepte Bl\u0061u', 'Gestión de piscinas')
    .replaceAll('Factura generada desde Gestión de piscinas.', 'Factura generada desde la aplicación.')
}
