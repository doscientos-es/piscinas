...
type Invoice = { id: string; number: string | null; status: string; total: number; issued_on: string | null; clients: { legal_name: string } | null };
...
export function DemoApp({ view }: { view: View }) {
  ...
  const load = useCallback(async () => { const s = createClient(); const [v, i, c] = await Promise.all([s.from("visits").select("id,scheduled_for,status,installations(name,address,clients(legal_name))").order("scheduled_for"), s.from("invoices").select("id,number,status,total,issued_on,clients(legal_name)").order("created_at", { ascending: false }), s.from("clients").select("id,legal_name,payment_method,installations(id)").order("legal_name")]); const error = v.error || i.error || c.error; if (error) { setMessage(error.message); return; } setVisits((v.data ?? []) as unknown as Visit[]); setInvoices((i.data ?? []) as unknown as Invoice[]); setClients((c.data ?? []) as unknown as Client[]); }, []);
